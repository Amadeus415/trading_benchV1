...
class EpisodeEngine:
    ...
    def apply_corporate_actions(self, on: date) -> None:
        ...
        for _, row in actions.iterrows():
            sym = row["symbol"]
            if row["action_type"] == "split" and row["ratio"] is not None and not pd.isna(row["ratio"]):
                self.ledger.apply_split(sym, float(row["ratio"]))
            elif row["action_type"] == "cash_dividend" and row["amount"] is not None and not pd.isna(row["amount"]):
                self.ledger.apply_dividend(sym, float(row["amount"]))
